...
import com.mrfuzzihead.archimedes.common.network.ClientOpenGuiMessage;
        ...
        if (e.phase == TickEvent.Phase.START && e.side == Side.CLIENT
            ...
            if (config.kbDisassemble.getIsKeyPressed() && !kbDisassemblePrevState) {
                MovingWorldClientActionMessage msg = new MovingWorldClientActionMessage(
                    (EntityShip) e.player.ridingEntity,
            ...
            if (c != ship.getController()
                .getShipControl()) {
                ship.getController()
                    .updateControl(ship, e.player, c);
            }
        }
    }
