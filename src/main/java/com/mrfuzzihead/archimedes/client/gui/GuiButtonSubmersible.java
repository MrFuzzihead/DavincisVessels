    ...
    public void drawButton(Minecraft mc, int mouseX, int mouseY) {
        if (this.visible) {
            mc.getTextureManager()
                .bindTexture(new ResourceLocation("archimedesships", "textures/gui/submerse.png"));
            ...
            if (mouseOver) {
                int stringWidth = Minecraft.getMinecraft().fontRenderer
                    .getStringWidth((submerse ? "Submerse Ship" : "Don't Submerse Ship"));
                drawHoveringText(
                    (submerse ? "Submerse Ship" : "Don't Submerse Ship"),
                    mouseX + (stringWidth / 2) + 32,
                    mouseY - 12,
