package com.mrfuzzihead.archimedes.client.gui;
...
import com.mrfuzzihead.archimedes.common.network.ClientHelmActionMessage;
import com.mrfuzzihead.archimedes.common.network.ClientRenameShipMessage;
...
public class GuiHelm extends GuiContainer {
    ...
    public void initGui() {
        ...
        btnAssemble = new GuiButton(
            1,
            btnx,
            btny += 20,
            100,
            20,
            StatCollector.translateToLocal("gui.shipstatus.compile"));
        buttonList.add(btnAssemble);

        btnUndo = new GuiButton(2, btnx, btny += 20, 100, 20, StatCollector.translateToLocal("gui.shipstatus.undo"));
        btnUndo.enabled = tileEntity.getPrevAssembleResult() != null && tileEntity.getPrevAssembleResult()
            .getCode() != AssembleResult.RESULT_NONE;
        buttonList.add(btnUndo);

        btnMount = new GuiButton(3, btnx, btny += 20, 100, 20, StatCollector.translateToLocal("gui.shipstatus.mount"));
        btnMount.enabled = tileEntity.getAssembleResult() != null && tileEntity.getAssembleResult()
            .getCode() == AssembleResult.RESULT_OK;
    ...
    protected void drawGuiContainerForegroundLayer(int mousex, int mousey) {
        ...
        if (result == null || (result != null && result.assemblyInteractor == null)
            || (result != null && result.assemblyInteractor != null
                && !(result.assemblyInteractor instanceof ShipAssemblyInteractor))) {
        ...
        fontRendererObj
            .drawString(StatCollector.translateToLocal("gui.shipstatus.compilerresult"), col0, row += 10, color);
        ...
        if (rblocks == 0) {
            ...
            fontRendererObj.drawString(
                StatCollector.translateToLocal(
                    balloonratio > ArchimedesShipMod.instance.modConfig.flyBalloonRatio ? "gui.shipstatus.type.airship"
                        : "gui.shipstatus.type.boat"),
                col1,
                row,
                color);
        }
        ...
        fontRendererObj
            .drawString(StatCollector.translateToLocal("gui.shipstatus.count.block"), col0, row += 10, color);
        fontRendererObj.drawString(String.valueOf(rblocks), col1, row, color);

        fontRendererObj
            .drawString(StatCollector.translateToLocal("gui.shipstatus.count.balloon"), col0, row += 10, color);
        fontRendererObj
            .drawString(String.valueOf(rballoons) + " (" + (int) (balloonratio * 100f) + "%)", col1, row, color);

        fontRendererObj
            .drawString(StatCollector.translateToLocal("gui.shipstatus.count.tileentity"), col0, row += 10, color);
        ...
        fontRendererObj.drawString(
            String.format(Locale.ROOT, "%.1f %s", rmass, StatCollector.translateToLocal("gui.shipstatus.massunit")),
            col1,
            row,
            color);
    }
    ...
    public void actionPerformed(GuiButton button) {
        if (button == btnRename) {
            if (txtShipName.isFocused()) {
                ...
                tileEntity.getInfo()
                    .setName(txtShipName.getText());
                txtShipName.setFocused(false);
                // txtShipName.setEnableBackgroundDrawing(false);

                ClientRenameShipMessage msg = new ClientRenameShipMessage(
                    tileEntity,
                    tileEntity.getInfo()
                        .getName());
    ...
    protected void keyTyped(char c, int k) {
        if (!checkHotbarKeys(k)) {
            ...
            } else if (txtShipName.textboxKeyTyped(c, k)) {} else {
                super.keyTyped(c, k);
            }
        }
    }
