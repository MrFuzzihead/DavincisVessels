...
import com.mrfuzzihead.movingworld.common.chunk.assembly.CanAssemble;
...
public class ShipAssemblyInteractor extends MovingWorldAssemblyInteractor {
    ...
    public boolean isBlockMovingWorldMarker(Block block) {
        if (block != null) return block.getUnlocalizedName() == ArchimedesObjects.blockMarkShip.getUnlocalizedName();
        else return false;
    }

    @Override
    public boolean isTileMovingWorldMarker(TileEntity tile) {
        if (tile != null) return tile instanceof TileEntityHelm;
        else return false;
    }
