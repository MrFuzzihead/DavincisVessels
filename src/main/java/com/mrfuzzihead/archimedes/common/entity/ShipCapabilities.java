...
import com.mrfuzzihead.archimedes.common.object.block.AnchorPointLocation;
...
import com.mrfuzzihead.movingworld.common.entity.MovingWorldCapabilities;
import com.mrfuzzihead.movingworld.common.util.FloodFiller;
import com.mrfuzzihead.movingworld.common.util.LocatedBlockList;
import com.mrfuzzihead.movingworld.common.util.MaterialDensity;
...
public class ShipCapabilities extends MovingWorldCapabilities {
    ...
    public float getEnginePower() {
        return ship.getDataWatcher()
            .getWatchableObjectFloat(29);
    }
            ...
            if (anchorPoints != null) {
                ...
                for (LocatedBlock anchorPointLB : anchorPoints) {
                    ...
                    if (validDistance && ship.worldObj.getTileEntity(infoPosX, infoPosY, infoPosZ) != null
                        ...
                        TileEntityAnchorPoint anchorPoint = (TileEntityAnchorPoint) ship.worldObj
                            .getTileEntity(infoPosX, infoPosY, infoPosZ);
                        if (anchorPoint.anchorPointInfo != null && !anchorPoint.anchorPointInfo.forShip) {
                            ...
                            ChunkPosition anchorPointCoords = new ChunkPosition(
                                anchorPoint.xCoord,
                                anchorPoint.yCoord,
                                anchorPoint.zCoord);

                            anchorPointLocation.worldAnchor = new LocatedBlock(
                                anchorPointWorld.getBlock(
                                    anchorPointCoords.chunkPosX,
                                    anchorPointCoords.chunkPosY,
                                    anchorPointCoords.chunkPosZ),
                                anchorPointWorld.getBlockMetadata(
                                    anchorPointCoords.chunkPosX,
                                    anchorPointCoords.chunkPosY,
                                    anchorPointCoords.chunkPosZ),
                                anchorPointCoords);
    ...
    public void updateEngines() {
        ...
        if (!ship.worldObj.isRemote) ship.getDataWatcher()
            .updateObject(29, ePower);
    }

    @Override
    public boolean canFly() {
        return (ArchimedesShipMod.instance.modConfig.enableAirShips
            && getBalloonCount() >= blockCount * ArchimedesShipMod.instance.modConfig.flyBalloonRatio)
            || ship.areSubmerged();
    }
    ...
    public boolean canSubmerge() {
        if (!submerseFound) {
            ...
            canSubmerge = false;
            if (ArchimedesShipMod.instance.modConfig.enableSubmersibles) canSubmerge = filledBlockCount
                < (nonAirBlockCount * ArchimedesShipMod.instance.modConfig.submersibleFillRatio);
            submerseFound = true;
        }
        return canSubmerge;
    }
    ...
    public void postBlockAdding() {
        if (ship.getMobileChunk() != null && ship.getMobileChunk().marker != null
            && ship.getMobileChunk().marker.tileEntity != null
            && ship.getMobileChunk().marker.tileEntity instanceof TileEntityHelm) {
    ...
    public void onChunkBlockAdded(Block block, int metadata, int x, int y, int z) {
        ...
        if (block == null) {} else if (block == ArchimedesObjects.blockFloater) {
            nonAirBlockCount--;
            return;
        }
        if (block instanceof BlockAir) nonAirBlockCount--;
        ...
        if (block == ArchimedesObjects.blockBalloon || ArchimedesShipMod.instance.modConfig.isBalloon(block)) {
            ...
            TileEntity te = ship.getMovingWorldChunk().chunkTileEntityMap.get(new ChunkPosition(x, y, z));
            if (te != null && te instanceof TileEntityAnchorPoint
                && ((TileEntityAnchorPoint) te).anchorPointInfo != null
                && ((TileEntityAnchorPoint) te).anchorPointInfo.forShip) {
                if (anchorPoints == null) {
                    anchorPoints = new ArrayList<LocatedBlock>();
                }
                anchorPoints
                    .add(new LocatedBlock(block, metadata, te, new ChunkPosition(x, y, z), new ChunkPosition(0, 0, 0)));
            }
        ...
        } else if ((block == ArchimedesObjects.blockSeat || ArchimedesShipMod.instance.modConfig.isSeat(block))
            && !ship.worldObj.isRemote) {
