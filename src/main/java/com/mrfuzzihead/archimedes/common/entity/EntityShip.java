...
import com.mrfuzzihead.archimedes.client.control.ShipControllerClient;
...
import com.mrfuzzihead.movingworld.common.chunk.assembly.ChunkDisassembler;
import com.mrfuzzihead.movingworld.common.chunk.assembly.MovingWorldAssemblyInteractor;
...
import com.mrfuzzihead.movingworld.common.entity.MovingWorldHandlerCommon;
...
public class EntityShip extends EntityMovingWorld {
        ...
        if (worldObj != null) {
            if (!worldObj.isRemote) {
                ...
                if (capabilities.getEngines() != null) {
                    if (capabilities.getEngines()
                        .isEmpty()) hasEngines = false;
                    else {
                ...
                else getDataWatcher().updateObject(28, new Byte((byte) 1));
    ...
    public void setSubmerge(boolean submerge) {
        ...
        if (worldObj != null && !worldObj.isRemote) {
            ...
            if (getMobileChunk().marker != null && getMobileChunk().marker.tileEntity != null
                && getMobileChunk().marker.tileEntity instanceof TileEntityHelm) {
    ...
    public AxisAlignedBB getCollisionBox(Entity entity) {
        return entity instanceof EntitySeat || entity.ridingEntity instanceof EntitySeat
            || entity instanceof EntityLiving ? null : entity.boundingBox;
    }
    ...
    public boolean alignToAnchor() {
        if (capabilities.findClosestValidAnchor(16) != null) {
            ...
            Vec3 worldPosForAnchor = Vec3
                .createVectorHelper(worldAnchorPos.chunkPosX, worldAnchorPos.chunkPosY + 2, worldAnchorPos.chunkPosZ);
            worldPosForAnchor = worldPosForAnchor
                .addVector(getMobileChunk().getCenterX(), getMobileChunk().minY(), getMobileChunk().getCenterZ());
            worldPosForAnchor = Vec3.createVectorHelper(
                worldPosForAnchor.xCoord - chunkAnchorPos.chunkPosX,
                worldPosForAnchor.yCoord,
                worldPosForAnchor.zCoord - chunkAnchorPos.chunkPosZ);
            ...
            if (prevRiddenByEntity != null) {
                if (ArchimedesShipMod.instance.modConfig.disassembleOnDismount) {
                    ...
                    if (!worldObj.isRemote && isFlying()) {
                        EntityParachute parachute = new EntityParachute(
                            worldObj,
                            this,
                            riderDestinationX,
                            riderDestinationY,
    ...
    private void handlePlayerControl() {
        if (riddenByEntity instanceof EntityLivingBase && ((ShipCapabilities) getCapabilities()).canMove()) {
            ...
            if (ArchimedesShipMod.instance.modConfig.shipControlType == ArchimedesConfig.CONTROL_TYPE_ARCHIMEDES) {
                ...
                motionYaw += steer * BASE_TURN_SPEED
                    * capabilities.getRotationMult()
                    * ArchimedesShipMod.instance.modConfig.turnSpeed;
