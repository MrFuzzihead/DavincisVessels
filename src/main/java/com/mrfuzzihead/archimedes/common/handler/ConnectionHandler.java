            ...
            if (event.player.ridingEntity != null && event.player.ridingEntity instanceof EntitySeat) {
                ...
                if (ship != null && seat.getPos() != null) {
                    ...
                    Vec3 vec = Vec3.createVectorHelper(
                        seat.getPos().chunkPosX - ship.getMovingWorldChunk()
                            .getCenterX(),
                        seat.getPos().chunkPosY - ship.getMovingWorldChunk()
                            .minY(),
                        seat.getPos().chunkPosZ - ship.getMovingWorldChunk()
                            .getCenterZ());
                    ...
                    player.getEntityData()
                        .setTag("parachuteInfo", nbt);
                    player.getEntityData()
                        .setBoolean("reqParachute", true);
                }
            }
        }
    }
    ...
    public void onPlayerLoggedIn(PlayerEvent.PlayerLoggedInEvent event) {
        ...
        if (event.player != null && event.player.worldObj != null && !event.player.worldObj.isRemote) {
            ...
            if (player.getEntityData()
                .getBoolean("reqParachute") == true) {
                NBTTagCompound nbt = player.getEntityData()
                    .getCompoundTag("parachuteInfo");
                ...
                player.getEntityData()
                    .removeTag("parachuteInfo");
                player.getEntityData()
                    .setBoolean("reqParachute", false);
            }
        }
    }
}
