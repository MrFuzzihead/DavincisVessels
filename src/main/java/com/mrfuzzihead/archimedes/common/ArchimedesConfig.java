...
public class ArchimedesConfig {
    ...
    public ArchimedesConfig(Configuration configuration) {
        ...
        FMLCommonHandler.instance()
            .bus()
            .register(this); // For in game config reloads.
    }

    public void loadAndSave() {
        config.load();

        shipEntitySyncRate = config.get(
            "settings",
            "sync_rate",
            20,
            ...
            .getInt();
        enableAirShips = config.get("settings", "enable_air_ships", true, "Enable or disable air ships.")
            .getBoolean(true);
        enableSubmersibles = config
            .get("settings", "enable_submersibles", true, "Enable or disable the ability to submerse ships.")
            .getBoolean(true);
        bankingMultiplier = (float) config.get(
            "settings",
            "banking_multiplier",
            3d,
            ...
            .getDouble(3d);
        enginesMandatory = config
            .get("settings", "mandatory_engines", false, "Are engines required for a ship to move?")
            .getBoolean();

        shipControlType = config
            .get(
                "control",
                "control_type",
                CONTROL_TYPE_ARCHIMEDES,
                "Set to 0 to use vanilla boat controls, set to 1 to use the new Archimedes controls.")
            .getInt();
        turnSpeed = (float) config.get("control", "turn_speed", 1D, "A multiplier of the ship's turn speed.")
            .getDouble(1D);
        speedLimit = (float) config
            .get(
                "control",
                "speed_limit",
                30D,
            ...
            .getDouble(30D);
        speedLimit /= 20F;
        disassembleOnDismount = config.get("control", "decompile_on_dismount", false)
            .getBoolean(false);

        maxShipChunkBlocks = config
            .get(
                "mobile_chunk",
                "max_chunk_blocks",
                2048,
        ...
        flyBalloonRatio = (float) config
            .get(
                "mobile_chunk",
                "airship_balloon_ratio",
                0.4D,
                "The part of the total amount of objects that should be balloon objects in order to make an airship.")
            .getDouble(0.4D);
        submersibleFillRatio = (float) config
            .get(
                "mobile_chunk",
                "submersible_fill_ratio",
                0.3D,
    ...
    public void postLoad() {
        Block[] defaultStickyBlocks = { ArchimedesObjects.blockStickyBuffer, Blocks.stone_button, Blocks.wooden_button,
            Blocks.lever };
        String[] stickyBlockNames = new String[defaultStickyBlocks.length];
        for (int i = 0; i < defaultStickyBlocks.length; i++) {
            stickyBlockNames[i] = Block.blockRegistry.getNameForObject(defaultStickyBlocks[i])
                .toString();
        }
        ...
        for (int i = 0; i < defaultSeatBlocks.length; i++) {
            seatBlockNames[i] = Block.blockRegistry.getNameForObject(defaultSeatBlocks[i])
                .toString();
        }
        ...
        for (int i = 0; i < defaultBalloonBlocks.length; i++) {
            balloonBlockNames[i] = Block.blockRegistry.getNameForObject(defaultBalloonBlocks[i])
                .toString();
        }

        config.load();

        String[] balloonBlocks = config
            .get(
                "mobile_chunk",
                "balloon_blocks",
                balloonBlockNames,
    ...
    private void loadKeybindings() {
        ...
        kbDown = new KeyBinding(
            "key.archimedes.down",
            getKeyIndex(config, "key_descent", Keyboard.KEY_Z),
            "Archimedes");
        kbBrake = new KeyBinding(
            "key.archimedes.brake",
            getKeyIndex(config, "key_brake", Keyboard.KEY_C),
            "Archimedes");
        kbAlign = new KeyBinding(
            "key.archimedes.align",
            getKeyIndex(config, "key_align", Keyboard.KEY_EQUALS),
            "Archimedes");
        kbDisassemble = new KeyBinding(
            "key.archimedes.decompile",
            getKeyIndex(config, "key_decompile", Keyboard.KEY_BACKSLASH),
            "Archimedes");
        kbShipInv = new KeyBinding(
            "key.archimedes.shipinv",
            getKeyIndex(config, "key_shipinv", Keyboard.KEY_K),
            "Archimedes");
        Minecraft mc = Minecraft.getMinecraft();
        mc.gameSettings.keyBindings = ArrayUtils
            .addAll(mc.gameSettings.keyBindings, kbUp, kbDown, kbBrake, kbAlign, kbDisassemble, kbShipInv);
    }

    @SideOnly(Side.CLIENT)
    private int getKeyIndex(Configuration config, String name, int defaultkey) {
        return Keyboard.getKeyIndex(
            config.get("control", name, Keyboard.getKeyName(defaultkey))
                .getString());
    }

    public boolean isBalloon(Block block) {
        return balloonAlternatives.contains(
            Block.blockRegistry.getNameForObject(block)
                .toString());
    }

    public boolean isSeat(Block block) {
        return seats.contains(
            Block.blockRegistry.getNameForObject(block)
                .toString());
    }

    public boolean isSticky(Block block) {
        return stickyObjects.contains(
            Block.blockRegistry.getNameForObject(block)
                .toString());
    }
    ...
    public void onConfigChange(ConfigChangedEvent.OnConfigChangedEvent event) {
        if (event.modID.equals(ArchimedesShipMod.MODID)) {
